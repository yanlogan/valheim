...
- **Trash** — корзина в UI + `Delete` (хотбар не трогает, если cfg как у хоста). **Sort выкл** (EPI quickslots / смерть).
- Place Stacks скрыт; dump лута — **Unload**.
...
- **AzuCraftyBoxes** — крафт/стройка тянут из сундуков **и настенных drawers** до ~**50 м**; HUD это учитывает. Пауза pull: обычно `O+LeftAlt` (свой хоткей в F1). V+ CraftFromChest у нас **выкл**. Без **Yanlo-CraftyBoxesDrawerFix** (из zip) drawers часто показывают `0/N`, хотя материалы там есть. **1.1.5+** ещё чинит крафт/reclaim пачкой: не уезжает на следующий рецепт в списке.
- **WardIsLove** — крафтится **Thorward** (ванильный ward не крафтится). У своего дома: сломай ward, который поставил хост → поставь **свой** на то же место → Interact → **Ward Range** = число на **знаке рядом** (в GUI «N%» ≈ радиус **N**, шкала 0–100). На общую зону (плавильни) ward не ставим. Если портал не работает и проходишь насквозь — поставь **Yanlo-PortalWardFix** из zip (баг WardIsLove: радиус последнего инициализированного варда ломает считывание близости портала; нужен на **клиенте**).
- **PlanBuild**:
