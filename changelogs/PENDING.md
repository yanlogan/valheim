...
- **Yanlo-PortalWardFix** **1.0.1** <sub>от Yanlo</sub> — **обязательно на клиенте** с WardIsLove. Чинит «хожу сквозь портал» / нет телепорта (баг WardIsLove: радиус последнего инициализированного варда ломает считывание близости портала).
- **Yanlo-CraftyBoxesDrawerFix** **1.1.5** <sub>от Yanlo</sub> — без него крафт часто пишет `0/N` на материалы в **настенных drawers** (в сундуках всё ок). С патчем drawers считаются / тянутся; AAA **Max** тоже. **1.1.5:** AAA amount>1 / reclaim больше не уезжает на следующий рецепт в списке. **1.1.4:** меньше просадка FPS в инвентаре/сундуке.
