# Friends cycle — PENDING

Status: **open** · started: **2026-08-19**  
Стек: [`docs/STACK.md`](../docs/STACK.md) · Геймплей: [`docs/HOWTO.md`](../docs/HOWTO.md) · Конфиги: [`cfg/`](../cfg/)

## Ссылки

- **Дельта (этот файл):** https://github.com/yanlogan/valheim/blob/main/changelogs/PENDING.md
- **Release:** https://github.com/yanlogan/valheim/releases/tag/cycle-2026-08-19
- **Предыдущий:** https://github.com/yanlogan/valheim/releases/tag/cycle-2026-08-18
- **Стек / HOWTO / cfg:** [STACK](https://github.com/yanlogan/valheim/blob/main/docs/STACK.md) · [HOWTO](https://github.com/yanlogan/valheim/blob/main/docs/HOWTO.md) · [cfg/](https://github.com/yanlogan/valheim/tree/main/cfg)

> Полный стек — в STACK; здесь дельта **cycle-2026-08-19**.

---

## Что изменилось

### Добавлено

**По желанию**

- **Glowing_Signs** <sub>от ituallovich</sub> — таблички светятся в темноте, чтобы читать ночью

### Удалено

- _(нет)_

### Конфиг

- _(нет)_

### Yanlo

- **Yanlo-CraftyBoxesDrawerFix** **1.1.8** — `E` на настенном ящике после того как печь/домна сама туда сложила: берёт один стак, остаток остаётся (раньше остаток пропадал). Перезапиши папку из zip. Только клиент; релог.

### Как пользоваться

См. [HOWTO](../docs/HOWTO.md#ui--qol-если-поставил-optional).

---

## Закрытие цикла

Только когда все друзья забрали апдейт («синк с друзьями»):

1. Этот файл → `changelogs/YYYY-MM-DD_slug.md`
2. `.\scripts\release.ps1` при новом zip/notes (или `gh release edit` для правок notes)
3. Новый PENDING из `_PENDING_TEMPLATE.md`
