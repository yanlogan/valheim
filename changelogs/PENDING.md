...
- **Yanlo-CraftyBoxesDrawerFix** **1.1.7** <sub>от Yanlo</sub> — без него крафт часто пишет `0/N` на материалы в **настенных drawers** (в сундуках всё ок). **1.1.7:** если крафт/Shift+E опустошил drawer — **тип предмета остаётся** (не как Alt+E). **1.1.6:** мельница / прялка / печь тянут из drawers (**Shift+E**). **1.1.5:** крафт/reclaim пачкой больше не уезжает на следующий рецепт.
...
- **AutoMapPins** (optional, только клиент): yaml в [`cfg/FixItFelix.AutoMapPins.categories.vanilla.yaml`](../cfg/FixItFelix.AutoMapPins.categories.vanilla.yaml) — радар: медь, дикие семена морковь/репа, ячмень/лён/molten/tar, живые гнёзда; лук-семена не пиним (сундуки в горах); thistle выкл; крипты+суртлинги навсегда. Скопируй файл в свой `BepInEx/config/`. Старые пины персонажа: сейв → меню → зайти (иногда два раза). Не `amp clear_pins`.
